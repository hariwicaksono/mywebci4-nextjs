  ...
  render() {
    const columns = [
      ...
      },
      {
        ...
        cell: (row) => (
          <>
            {row.post_image != null ? (
              <img
                src={this.state.url + row.post_image}
                width="60"
                alt=""
                onClick={() => {
                  this.dialog.show({
              ...
              {({ handleSubmit, handleChange, handleBlur, values, touched, errors, isSubmitting }) => (
                <Form onChange={handleSubmit}>
                  <Form.Control as="select" name="category_id" className="form-select form-select-sm" onChange={handleChange} defaultValue={row.category_id} onBlur={handleBlur} size="sm">
                    {this.state.Category.map((b, i) => (
                      <option value={b.id} key={i}>
                        {isSubmitting ? "menunggu.." : b.name}
                      </option>
                    ))}
                  </Form.Control>
                </Form>
      ...
      },
      {
        ...
        cell: (row) => (
          <>
            <Link href={"/admin/blog/edit/" + row.id} passHref>
